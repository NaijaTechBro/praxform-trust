...
const CardIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="14" x="2" y="5" rx="2"/><line x1="2" x2="22" y1="10" y2="10"/></svg>
);
...
const BillingTab = () => {
    ...
    // --- FIX: REMOVED the isVerifying state. We will no longer show a blocking loader. ---
    // const [isVerifying, setIsVerifying] = useState(searchParams.get('success') === 'true');
    ...
    useEffect(() => {
        ...
        if (hasSuccessParam && orgId) {
            // --- STEP 1: Give INSTANT feedback, then remove the URL param ---
            toast.info('Payment successful! Your plan is being upgraded now.');
            
            const newParams = new URLSearchParams(searchParams);
            newParams.delete('success');
            setSearchParams(newParams, { replace: true });

            // --- STEP 2: Start polling SILENTLY in the background ---
            let attempts = 0;
            ...
            const intervalId = setInterval(async () => {
                ...
                try {
                    ...
                    if (updatedOrg && updatedOrg.plan !== 'starter') {
                        // --- STEP 3: Polling succeeded, give final confirmation ---
                        clearInterval(intervalId);
                        toast.success(`Your plan has been upgraded to the ${updatedOrg.plan} plan!`);
                    } else if (attempts >= maxAttempts) {
                        // Polling timed out
                        clearInterval(intervalId);
                        toast.warn("Your plan is updating. If you don't see it yet, please refresh in a moment.");
                    }
                } catch (error) {
                    // Polling failed
                    clearInterval(intervalId);
                    toast.error("An issue occurred while verifying your plan. Please contact support if the issue persists.");
                }
            }, 2000);
    ...
    // --- The component now renders its normal UI immediately ---
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 sm:p-8">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-2">Billing & Plan</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Manage your subscription and billing details.</p>
