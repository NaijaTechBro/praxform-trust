...
const BillingTab = () => {
    const { organization, fetchOrganizationDetails } = useOrganization();
    ...
    useEffect(() => {
        ...
        if (hasSuccessParam && orgId) {
            ...
            const intervalId = setInterval(async () => {
                attempts++;
                try {
                    const updatedOrg = await fetchOrganizationDetails(orgId);
                    ...
                    if (updatedOrg && updatedOrg.plan !== 'starter') {
                        ...
                        clearInterval(intervalId);
                        setIsVerifying(false);
                        toast.info("Your plan is updating. Please refresh in a moment to see the changes.");
                    }
                } catch (error) {
                    clearInterval(intervalId);
                    setIsVerifying(false);
                    toast.error("An error occurred while verifying your plan.");
    ...
    }, [organization?._id, searchParams, setSearchParams, fetchOrganizationDetails]);
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 sm:p-8">
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                {/* JSX for displaying plan details */}
