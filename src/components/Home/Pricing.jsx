...
const Pricing = () => {
    const { plans, getPlans, createCheckoutSession, loading: plansLoading } = usePayments();
    const [pricingTier, setPricingTier] = useState('annual');
    const [processingPlan, setProcessingPlan] = useState(null);

    const getPlansCallback = useCallback(getPlans, []);
    ...
    const handleGetStarted = async (priceId) => {
        // Assuming 'price_1S8IcXFnPEdPZtOZOKjBDgTE' is still your Starter/Free plan's priceId
        if (priceId === 'price_1S8IcXFnPEdPZtOZOKjBDgTE') { 
            alert("The Starter plan is free! You can get started by signing up.");
            return;
        }
