...
const FooterLink = ({ href, children }) => {
  ...
  if (href.startsWith('#')) {
    return (
      <li>
        <Link smooth to={`/${href}`} className="text-gray-300 hover:text-white transition-colors duration-200">
          {children}
        </Link>
      </li>
    );
  ...
  return (
    <li>
      <Link to={href} className="text-gray-300 hover:text-white transition-colors duration-200">
        {children}
      </Link>
    </li>
  );
    ...
    <footer id='support' className="bg-[#083166] text-white overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto pt-40 pb-12 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8">
          ...
          {/* Links Columns (No changes needed here as FooterLink handles the logic) */}
          <div>
