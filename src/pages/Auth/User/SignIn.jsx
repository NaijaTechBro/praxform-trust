...
const SignIn = () => {
    ...
    // Get `user` from context to check for existing sessions
    const { login, googleAuth, loading, user } = useAuth();
    const navigate = useNavigate();

    // --- IMPORTANT: This effect redirects users who are already logged in ---
    useEffect(() => {
        if (user) {
    ...
    const handleSignIn = async (e) => {
        ...
        // --- UPDATED LOGIC: Handles all successful login scenarios ---
        if (result.success && result.mfaRequired) {
            // Case 1: Success + MFA required -> Go to MFA code page
            navigate('/login-code', { state: { email } });
        } else if (result.success) {
            // Case 2: Success + NO MFA required -> Go to Dashboard
            toast.success("Login successful!");
            navigate('/dashboard');
        } else {
            // Case 3: Any failure -> Show an error
    ...
    // If a user is already logged in, show a loading/redirecting message
    if (user) {
        return (
